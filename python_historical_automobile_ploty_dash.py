...
# Plot 4 bar chart for the effect of unemployment rate on vehicle type and sales
        unemp_sales = recession_data.groupby('Vehicle_Type')['unemployment_rate'].mean().reset_index()
        R_chart4 = dcc.Graph(
            figure=px.bar(
                unemp_sales,
                x='Vehicle_Type',
...
# Plot 2 Total Monthly Automobile sales using line chart.
        ...
        exp_vdata = yearly_data.groupby('Vehicle_Type')['Advertising_Expenditure'].mean().reset_index()
        Y_chart4 = dcc.Graph(
            figure=px.pie(
                exp_vdata,
                values='Advertising_Expenditure',
                names='Vehicle_Type',
                title='Advertisement Expenditure for each vehicle type in the year {}'.format(input_year)
            )
        )
